...
} from "@/redux/api/emailDirectoryApi";
import { Trash2, Edit, Upload, Plus, Download, FileText, User, Calendar, Mail, Search, Filter, Lock, X } from "lucide-react";
import { toast } from "react-hot-toast";
import { useDropzone } from "react-dropzone";

const EmailDirectoryPage: React.FC = () => {
  ...
  const [bulkImportEmail] = useBulkImportEmailMutation();

  // Add/Edit Modal State
  const [showAddModal, setShowAddModal] = useState(false);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [form, setForm] = useState({ name: "", email: "", isConfidential: false });
  ...
  const handleAddOrUpdate = async () => {
    ...
    try {
      ...
      setForm({ name: "", email: "", isConfidential: false });
      setEditingId(null);
  ...
  const onDrop = async (acceptedFiles: File[]) => {
    ...
    try {
      await bulkImportEmail(file).unwrap();
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          ...
          <p className="text-gray-400 mt-4">Loading email directory...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center">
        <div>
          ...
          <p className="text-gray-400">
            Manage your email contacts for bulk email campaigns
          </p>
        </div>
      ...
      <div className="bg-gray-800 rounded-xl p-6 border border-gray-700">
        <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
          <div className="relative flex-1 w-full">
            ...
            <input
              type="text"
              placeholder="Search emails by name or email address..."
              value={searchTerm}
        ...
        {filteredEmails.length > 0 && (
          <div className="px-6 py-4 border-t border-gray-700 bg-gray-700/30">
            <div className="flex flex-col md:flex-row justify-between items-center text-sm text-gray-400">
              <div>
                Showing {filteredEmails.length} of {data?.length} emails
        ...
        <>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="bg-gray-800 rounded-2xl border border-gray-700 w-full max-w-md relative shadow-2xl">
              {/* Header */}
              <div className="flex items-center justify-between p-6 border-b border-gray-700">
                <h2 className="text-xl font-bold text-white">Bulk Upload Emails</h2>
                <button
                ...
                <div className="mt-6 p-4 bg-gray-700/50 rounded-lg border border-gray-600">
                  ...
                  <ul className="text-gray-400 text-sm space-y-2">
                    <li className="flex items-center gap-2">
                      ...
                      CSV format with columns: <code className="bg-gray-600 px-1 rounded">name, email, isConfidential</code>
                    </li>
                    <li className="flex items-center gap-2">
                      <div className="w-1.5 h-1.5 bg-yellow-500 rounded-full"></div>
...
export default EmailDirectoryPage;